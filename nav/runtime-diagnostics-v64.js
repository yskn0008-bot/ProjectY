...
(() => {
  ...
  const snapshot = () => {
    ...
    const pageNetworkMarkerValid = /^network-v\d+$/.test(PAGE_NETWORK_MARKER);
    const pageNetworkMarkerCurrent = Boolean(PAGE_NETWORK_MARKER && PAGE_NETWORK_MARKER === swStatus.networkSourceValue);
    const pageNetworkMarkerStale = pageNetworkMarkerValid && Boolean(swStatus.networkSourceValue) && !pageNetworkMarkerCurrent;
    const checks = {
      online: navigator.onLine,
      swControlled: swStatus.controlled,
      swBuild: swStatus.build || '未取得',
      swCache: swStatus.cache || '未取得',
      swServingCache: swStatus.servingCache || 'なし',
      swServingFallback: swStatus.servingFallback,
      swRetainedPreviousCache: swStatus.retainedPreviousCache || 'なし',
      swPinnedClientCount: swStatus.pinnedClientCount,
