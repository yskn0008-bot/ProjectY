'use strict';
(()=>{
  if(window.__yosNavRuntimeDiagnosticsV78)return;
  window.__yosNavRuntimeDiagnosticsV78=true;

  const BUILD='v78';
  const EXPECTED_CACHE='yos-navi-strategy-v78-content-signature-validation';
  ...
  const emptyStatus=(controlled,issue)=>({controlled,cache:null,buildMatch:false,offlineReady:false,missingCriticalAssets:issue?[issue]:[],invalidCriticalAssets:[]});
  const requestServiceWorkerStatus=()=>new Promise(resolve=>{
    ...
    channel.port1.onmessage=event=>{
      ...
      finish({controlled:true,cache,buildMatch:cache===EXPECTED_CACHE,offlineReady:event.data?.offlineReady===true,missingCriticalAssets,invalidCriticalAssets});
    };
  ...
  const snapshot=()=>{
    ...
    return Object.freeze({build:BUILD,checkedAt:new Date().toISOString(),ready:requiredReady&&(!checks.online||checks.tileReady),checks});
  };
  ...
  const renderPanel=report=>{if(!isDiagnosticMode)return;const signature=JSON.stringify({ready:report.ready,checks:report.checks});if(signature===lastPanelSignature)return;lastPanelSignature=signature;let panel=document.getElementById('yos-nav-diagnostics-v64');if(!panel){panel=document.createElement('section');panel.id='yos-nav-diagnostics-v64';panel.setAttribute('aria-live','polite');panel.style.cssText='position:fixed;z-index:99999;left:8px;right:8px;bottom:calc(env(safe-area-inset-bottom) + 8px);max-height:44vh;overflow:auto;padding:12px;border:1px solid #31516b;border-radius:14px;background:rgba(3,10,16,.96);color:#eaf7ff;font:12px/1.5 -apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 12px 30px rgba(0,0,0,.45)';document.body.appendChild(panel);}const rows=Object.entries(report.checks).map(([key,value])=>`<div><b>${key}</b>: ${String(value)}</div>`).join('');panel.innerHTML=`<div style="display:flex;justify-content:space-between;gap:8px"><strong>YOSナビ 診断 ${report.build}</strong><span>${report.ready?'READY':'CHECK'}</span></div>${rows}`;};
  ...
  window.addEventListener('online',()=>schedule(true));window.addEventListener('offline',()=>schedule(false));window.addEventListener('pageshow',()=>schedule(true));window.addEventListener('yos-nav-recommendation',()=>schedule(false));document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule(true);});schedule(true);
})();
