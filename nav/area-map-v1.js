...
(()=>{
  ...
  style.textContent=`
    .yos-area-map{margin:12px 0 0;padding:13px;border:1px solid var(--line);border-radius:20px;background:linear-gradient(155deg,rgba(111,168,255,.11),rgba(23,23,25,.98) 58%);overflow:hidden}
    .yos-area-map__head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:10px}
    .yos-area-map__head b{display:block;font-size:18px}.yos-area-map__head small{display:block;margin-top:4px;color:var(--muted);font-size:11px;line-height:1.45}
  ...
  const zoneMarkup=areas.map(area=>{
    ...
    return `<g class="yos-area-map__zone ${area.tone}" data-key="${area.key}" role="button" tabindex="0" aria-label="${escapeHtml(area.label)} 参考スコア ${area.score}" aria-pressed="false" transform="translate(${point.x.toFixed(1)} ${point.y.toFixed(1)})"><circle class="halo" r="29"></circle><circle class="core" r="21"></circle><text class="score" y="5">${area.score}</text><text class="name" y="38">${escapeHtml(area.label)}</text></g>`;
  }).join('');
  ...
  section.innerHTML=`
    <div class="yos-area-map__head"><div><b>営業エリアマップ</b><small>候補エリアの位置と優先度をひと目で確認</small></div><div class="yos-area-map__tools"><span class="yos-area-map__badge">概略・参考表示</span><button type="button" class="yos-area-map__toggle" aria-expanded="true">地図を閉じる</button></div></div>
  ...
  const openMaps=destination=>{
    ...
    const latitude=source?.dataset.latitude;
    const longitude=source?.dataset.longitude;
    const accuracy=Number(source?.dataset.accuracy||NaN);
    const acquiredAt=Number(source?.dataset.acquiredAt||0);
    ...
    url.searchParams.set('api','1');url.searchParams.set('destination',value);url.searchParams.set('travelmode','driving');url.searchParams.set('dir_action','navigate');
  ...
  toggle.addEventListener('click',()=>{
    ...
    toggle.textContent=collapsed?'地図を開く':'地図を閉じる';
  });
  selectArea('primary');
  ...
  const syncCurrentLocation=()=>{
    ...
    if(!Number.isFinite(latitude)||!Number.isFinite(longitude)||!inBounds(latitude,longitude)){
      currentMarker.classList.remove('is-visible');currentMarker.setAttribute('aria-hidden','true');return;
    }
    const point=project(latitude,longitude);
    currentMarker.setAttribute('transform',`translate(${point.x.toFixed(1)} ${point.y.toFixed(1)})`);
    currentMarker.classList.add('is-visible');currentMarker.setAttribute('aria-hidden','false');
  };
  syncCurrentLocation();
