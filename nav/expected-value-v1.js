...
(()=>{
  ...
  function rank(now=new Date()){
    ...
    for(const [zone,profile] of Object.entries(MODEL.zoneProfiles)){
      ...
      // Sparse segments are allowed as nearby alternatives, but cannot present
      // themselves with the same certainty as repeated results.
  ...
  style.textContent=`
    ...
    .yos-ev__head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}
    ...
    .yos-ev__cruise{flex:none;min-height:36px;padding:0 10px;border:1px solid var(--line);border-radius:999px;background:#222226;color:var(--text);font-size:10px;font-weight:950}
    .yos-ev__cruise.is-on{border-color:rgba(244,200,77,.62);background:rgba(244,200,77,.14);color:#ffe48c}
    ...
    @media(max-width:360px){.yos-ev__head{display:grid}.yos-ev__cruise{justify-self:start}}
  `;
  ...
  section.innerHTML=`
    <div class="yos-ev__head"><div><b>日報学習モデル</b><small class="yos-ev__meta"></small></div><button type="button" class="yos-ev__cruise"></button></div>
  ...
  function apply(){
    ...
    const location=currentLocation();
    meta.textContent=`確定${MODEL.confirmedClassifiedRides}件｜${MODEL.sourcePeriod.from.slice(5).replace('-','/')}〜${MODEL.sourcePeriod.to.slice(5).replace('-','/')}｜${dayGroup(now)==='weekday'?'月〜木':'金〜日'} ${timeBin(now)}`;
    ...
    list.innerHTML=top.map((item,index)=>{
      const basis=item.fieldRule
        ?`現場経験補正・統計${MODEL.zoneProfiles.PARCO.overall?.n||2}件`
    ...
    window.dispatchEvent(new CustomEvent('yos-nav-recommendation',{detail:{recommendations:top,cruiseActive}}));
  }
