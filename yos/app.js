...
(() => {
  ...
  const profile = read(PROFILE_KEY, { name: "", onboarded: false, createdAt: "" });
  let selectedStage = "";
  ...
  function journeyState() {
    ...
    return {
      chapter: value.chapter || "第1章｜旅の始まり",
      chapterMessage: value.chapterMessage || "人生を一気に変えなくていい。今日の一歩から物語は動き出す。",
      stage: value.stage || "日常世界",
      calling: value.calling || "",
      mainQuest: value.mainQuest || "",
      selectedXp: Number(value.selectedXp) || 10,
  ...
  const saveJourney = (journey) => write(JOURNEY_KEY, journey);
  ...
  function openOnboarding(force = false) {
    const journey = journeyState();
    $("heroName").value = force ? profile.name : profile.name || "";
    $("callingInput").value = force ? journey.calling : journey.calling || "";
    selectStage(journey.stage || "冒険への誘い");
    try { $("onboardingDialog").showModal(); }
    catch { $("onboardingDialog").setAttribute("open", ""); }
  }
  ...
  function saveOnboarding(event) {
    ...
    if (journey.chapter === "第1章｜旅の始まり" && name) journey.chapter = `第1章｜${name}の旅の始まり`;
    const saved = write(PROFILE_KEY, profile) && saveJourney(journey);
  ...
  function addQuickQuest(event) {
    ...
    const journey = journeyState();
    journey.quests.unshift({
      id: globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`,
      title, xp: 10, createdAt: new Date().toISOString()
    });
  ...
  function saveSettings(event) {
    ...
    const taxi = read(TAXI_KEY, {});
    taxi.yosUrl = url;
    const saved = write(PROFILE_KEY, profile) && saveJourney(journey) && write(HOME_KEY, settings) && write(TAXI_KEY, taxi);
  ...
  $("consultState").addEventListener("click", () => { saveCurrentState(); openYos(mentorPrompt("この体力・気持ち・違和感を前提に、無理なく進める一歩を一つ決めて。")); });
