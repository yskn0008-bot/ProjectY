...
test('P0 navigation and MY WAY identity are present',()=>{
  ...
  for(const label of ['Home','Life','Money','Hero’s Journey','Idea','過去の振り返り','記録 / ログ','目標 / 未来','過去の資産','Taxi','設定','System / 開発・状態','YOS改善','ヘルプ']) assert.ok(html.includes(label),label);
...
test('final five-domain UI uses facts without fabricated Money values',()=>{
  ...
  assert.match(js,/life\?\.moneySafety\s*\|\|\s*today\?\.money\s*\|\|\s*\{\}/);
