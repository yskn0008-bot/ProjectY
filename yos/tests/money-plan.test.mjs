...
function plan(liquid,transactions){
 const ctx={data:{transactions},isoToday:()=> '2026-09-19',monthKey:()=> '2026-09',currentLiquid:()=>liquid,isOutgoing:t=>t.type==='expense',txSign:t=>t.type==='income'?1:-1,n:Number,parseDate:s=>new Date(s+'T12:00:00+09:00'),daysBetween:(a,b)=>Math.ceil((b-a)/86400000),Date};
...
test('unknown balance preserves known payment and income without inventing forecast',()=>{const p=plan(null,tx);assert.equal(p.nextPayment.id,'p');assert.equal(p.nextIncome.id,'i');assert.equal(p.daysToNextPayment,7);assert.equal(p.projected,null);assert.equal(p.daily,null)});
