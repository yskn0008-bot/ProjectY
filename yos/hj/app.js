...
const defaults=[
{id:'work',name:'仕事',icon:'💼',stage:STAGES[0],stageUnknown:true,cycle:1,theme:'',updatedAt:''},
{id:'life',name:'生活',icon:'🏠',stage:STAGES[0],stageUnknown:true,cycle:1,theme:'',updatedAt:''},
{id:'money',name:'お金',icon:'¥',stage:STAGES[0],stageUnknown:true,cycle:1,theme:'',updatedAt:''},
{id:'relations',name:'人間関係',icon:'🤝',stage:STAGES[0],stageUnknown:true,cycle:1,theme:'',updatedAt:''},
{id:'dream',name:'夢・挑戦',icon:'✨',stage:STAGES[0],stageUnknown:true,cycle:1,theme:'',updatedAt:''}
];
...
function normalizeReality(value){const source=value&&typeof value==='object'?value:{};return{body:clean(source.body,120),mind:clean(source.mind,120),time:clean(source.time,120),money:clean(source.money,120),relationships:clean(source.relationships,120),environment:clean(source.environment,120)}}
function normalizeArchetypes(value){const source=value&&typeof value==='object'?value:{},active=Array.isArray(source.active)?source.active:[];return{active:[...new Set(active.filter((id)=>ARCHETYPE_IDS.has(id)))].slice(0,3),needed:ARCHETYPE_IDS.has(source.needed)?source.needed:'',balance:['helping','overused','unknown'].includes(source.balance)?source.balance:'',note:clean(source.note,400)}}
function normalizeJourney(value){if(!value||typeof value!=='object')return null;const name=clean(value.name,24);if(!name)return null;return{id:clean(value.id,100)||uid(),name,icon:clean(value.icon,4)||'🧭',stage:STAGES.includes(value.stage)?value.stage:STAGES[0],stageUnknown:Boolean(value.stageUnknown),cycle:Math.max(1,Math.min(99,Number(value.cycle)||1)),theme:clean(value.theme,160),compass:clean(value.compass,180),quest:clean(value.quest,180),treasure:clean(value.treasure,400),controllable:clean(value.controllable,400),reality:normalizeReality(value.reality),archetypes:normalizeArchetypes(value.archetypes),updatedAt:clean(value.updatedAt,60)}}
...
card.innerHTML=`<div class="journey-title"><span>${journey.icon}</span><div><strong>${escapeHtml(journey.name)}</strong><small>${journey.stageUnknown?'現在地は分からない':escapeHtml(journey.stage)} · 螺旋${journey.cycle}周目</small></div></div><div class="journey-grid"><label>現在のステージ<select data-role="stage"${journey.stageUnknown?' disabled':''}>${stageOptions(journey.stage)}</select></label><label>周回<input data-role="cycle" type="number" min="1" max="99" value="${journey.cycle}"></label></div><label class="check-label theme-field"><input data-role="stage-unknown" type="checkbox"${journey.stageUnknown?' checked':''}>今は現在地が分からない</label><label class="theme-field">今回のテーマ<input data-role="theme" maxlength="160" value="${escapeAttr(journey.theme)}" placeholder="例：自分に合う働き方を作る"></label><div class="card-actions"><button class="secondary" type="button" data-role="save">保存</button></div>`;
card.querySelector('[data-role="stage-unknown"]').addEventListener('change',(event)=>{card.querySelector('[data-role="stage"]').disabled=event.target.checked});
card.querySelector('[data-role="save"]').addEventListener('click',()=>{journey.stage=card.querySelector('[data-role="stage"]').value;journey.stageUnknown=card.querySelector('[data-role="stage-unknown"]').checked;journey.cycle=Math.max(1,Math.min(99,Number(card.querySelector('[data-role="cycle"]').value)||1));journey.theme=clean(card.querySelector('[data-role="theme"]').value,160);journey.updatedAt=nowIso();saveJourneys()});
list.appendChild(card);
});
}
function renderSummary(){
const box=$('journeySummary');box.innerHTML='';
journeys.forEach((journey)=>{const row=document.createElement('article');row.className='summary-row';row.innerHTML=`<span class="icon">${journey.icon}</span><div><strong>${escapeHtml(journey.name)}</strong><small>${escapeHtml(journey.theme||'テーマ未設定')}</small></div><b>${journey.stageUnknown?'分からない':escapeHtml(journey.stage)}</b>`;box.appendChild(row)});
$('journeyCount').textContent=journeys.length;const profile=read('hj-user-profile-v1',{}),focused=journeys.find((journey)=>journey.id===profile?.focusDomain)||journeys[0];$('activeStage').textContent=focused?(focused.stageUnknown?'分からない':focused.stage):'—';$('storyCount').textContent=stories.length;
}
