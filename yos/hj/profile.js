...
function stageOptions(selected,stageUnknown=false){
  const unknown=`<option value=""${stageUnknown?' selected':''}>分からない</option>`;
  return unknown+STAGES.map((stage)=>`<option${!stageUnknown&&stage===selected?' selected':''}>${escapeHtml(stage)}</option>`).join('');
}
...
function paintProfile(){
  ...
  if($('heroTitle'))$('heroTitle').textContent=data?.name?`${data.name}の現在地から、次の一手を見る。`:'現在地を知る。次の一手が見えてくる。';
  if($('heroCopy')&&focus)$('heroCopy').textContent=focus.stageUnknown?`今注目しているのは「${focus.name}」の旅。現在地はまだ分からなくて大丈夫です。`:`今注目しているのは「${focus.name}」の旅。物語上の現在地は「${focus.stage}」。前後や停滞に優劣はありません。`;
  if($('activeStage')&&focus)$('activeStage').textContent=focus.stageUnknown?'分からない':focus.stage;
}
function openDialog(force=false){
  ...
  $('profileStage').innerHTML=stageOptions(focus?.stage||STAGES[0],Boolean(focus?.stageUnknown));
...
function saveProfile(event){
  ...
  if(focus){
    const selectedStage=$('profileStage').value;
    if(selectedStage)focus.stage=selectedStage;
    focus.stageUnknown=!selectedStage;
...
function buildPrompt(){
  ...
  const lines=items.map((journey)=>journey.stageUnknown
    ? `- ${journey.name}：現在地は分からない${journey.theme?`・テーマ「${journey.theme}」`:''}`
    : `- ${journey.name}：「${journey.stage}」・螺旋${journey.cycle||1}周目${journey.theme?`・テーマ「${journey.theme}」`:''}`);
