...
(()=>{
  ...
  async function loadPublicConfig(){const response=await fetch(`${API}/api/yos/public-config`,{headers:{Accept:'application/json'},cache:'no-store',credentials:'omit'});if(!response.ok)throw new Error('config');return await response.json()}
  function loadGoogleScript(){return new Promise((resolve,reject)=>{if(globalThis.google?.accounts?.id)return resolve();const existing=document.querySelector('script[data-yos-tasks-google]');if(existing){existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',reject,{once:true});return}const s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.async=true;s.defer=true;s.dataset.yosTasksGoogle='1';s.onload=resolve;s.onerror=reject;document.head.append(s)})}
  async function setupAuth(autoPrompt=true){
    try{const config=await loadPublicConfig();await loadGoogleScript();globalThis.google.accounts.id.initialize({client_id:config.googleClientId,callback:async response=>{credential=String(response?.credential||'');if(!credential)return;try{await loadTasks(credential)}catch{showAuth('更新できませんでした。もう一度読み込んでください。')}},auto_select:false,cancel_on_tap_outside:true,use_fedcm_for_prompt:true});initialized=true;if(autoPrompt)globalThis.google.accounts.id.prompt()}catch{showAuth('タスクを更新するにはGoogle本人確認が必要です。')}
  }
  ...
  function install(){
    ...
    const cached=readCache();if(cached){const age=Date.now()-Number(cached.savedAt||0);render(cached.data,age<CACHE_MAX_MS?'前回の状態':'更新待ち')}else showAuth('最初にGoogle本人確認をすると、現在のタスクが表示されます。');setupAuth(Boolean(cached));
  }
