...
try {
  ...
  assert.match(serviceWorkerSource, /const\s+CACHE_PREFIX\s*=\s*['"]yos-command-center-['"];/, 'YOS専用キャッシュprefixが定義されていない');
  assert.match(serviceWorkerSource, /\.filter\(\(key\)\s*=>\s*key\.startsWith\(CACHE_PREFIX\)\s*&&\s*key\s*!==\s*CACHE\)/, 'YOS以外のキャッシュまで削除する可能性がある');
