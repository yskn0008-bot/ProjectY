...
assert.match(source, /legacy\.append\(layout,sunrise,week\)/, 'legacy renderer targets must leave the visible app together');
assert.match(source, /legacy\.hidden=true/, 'legacy view must stay hidden');
assert.match(source, /legacy\.inert=true/, 'legacy view must not accept interaction');
