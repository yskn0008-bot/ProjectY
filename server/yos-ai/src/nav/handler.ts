...
import {buildNavModel, type NavModel} from './model.js';
...
export function createNavModelHandler(options: CreateNavModelHandlerOptions): (request: Request) => Promise<Response> {
  ...
  let inFlight: Promise<NavModel> | null = null;
  ...
  async function loadModel(request: Request): Promise<NavModel> {
    ...
    return buildNavModel(rows, clock());
  }
}
...
function modelResponse(model: NavModel, origin: string, source: string, stale = false): Response {
