...
import {createYosCaptureProcessor} from '../intake/yos-capture-processor.js';
...
export function createProductionSlackEventsHandler(options: {
  ...
  const rawFirst = createRawFirstProcessor({
  ...
  const processor = createYosCaptureProcessor({rawFirst, redis});
