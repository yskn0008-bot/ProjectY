#!/usr/bin/env python3
"""Verify writes made by the Clarity Demo execution child before APPLIED.

This variant derives the expected title directly from each writer action rather
than from a shared content variable. That keeps Calendar, Reminder and Shopping
verification correct when a structured action also carries notes or conditions.
"""
...
def patch(path: Path) -> None:
    ...
    for index, kind, writer in reversed(writers):
        ...
        title_expected = params.get("WFCalendarItemTitle")
        if title_expected is None:
            raise SystemExit(f"{kind} writer missing title")
        ...
        checks.append((uid, title_name, 4, copy.deepcopy(title_expected), f"{kind}_title_mismatch"))
    ...
    print("Clarity Execute Actions readback verification patch: PASS")
