...
// Title opens YOS AC Remote. Medium widget exposes only frequent physical-remote functions.
...
async function getClientAndState(){
  ...
  if(!Number.isFinite(s.P))s.P=0;if(!Number.isFinite(s.M))s.M=0;if(!Number.isFinite(s.T))s.T=26;if(!Number.isFinite(s.S))s.S=0;if(!Number.isFinite(s.D))s.D=6;
...
const ACTIONS={cool:{label:'冷房',icon:'snowflake'},dry:{label:'除湿',icon:'drop'},heat:{label:'暖房',icon:'flame'},tempDown:{label:'温度−',icon:'minus'},tempUp:{label:'温度＋',icon:'plus'},stop:{label:'停止',icon:'power'}};
function addButton(row,name){const spec=ACTIONS[name];const s=row.addStack();s.layoutVertically();s.centerAlignContent();s.cornerRadius=16;s.backgroundColor=new Color('#191919');s.setPadding(8,6,7,6);s.url=runURL(name);const img=s.addImage(SFSymbol.named(spec.icon).image);img.imageSize=new Size(22,22);img.tintColor=new Color(name==='stop'?'#FF453A':'#0A84FF');s.addSpacer(4);const t=s.addText(spec.label);t.font=Font.semiboldSystemFont(13);t.textColor=Color.white();t.centerAlignText();}
function makeWidget(){const w=new ListWidget();w.backgroundColor=new Color('#000000');w.setPadding(12,12,12,12);const h=w.addStack();h.centerAlignContent();h.url=remoteURL();const title=h.addText('エアコン');title.font=Font.boldSystemFont(18);title.textColor=Color.white();h.addSpacer();const sub=h.addText('SHARP');sub.font=Font.systemFont(11);sub.textColor=new Color('#8E8E93');w.addSpacer(8);for(const names of [['cool','dry','heat'],['tempDown','tempUp','stop']]){const row=w.addStack();row.layoutHorizontally();names.forEach((n,i)=>{if(i)row.addSpacer(7);addButton(row,n);});if(names[0]==='cool')w.addSpacer(7);}return w;}
async function main(){const action=args.queryParameters&&args.queryParameters.action;if(action){try{await runAction(action);}catch(e){const a=new Alert();a.title='エアコン';a.message=e.message||String(e);a.addAction('OK');await a.presentAlert();}Script.complete();return;}const w=makeWidget();if(config.runsInWidget)Script.setWidget(w);else await w.presentMedium();Script.complete();}
await main();
