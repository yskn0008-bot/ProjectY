...
function uiState(){return {power:!!state.P,mode:modeName(state),temp:state.T,dry:state.M===4};}
...
async function perform(action){
  ...
  else if(action==='fan'){if(state.M===MODES.dry)return;await send({P:1,S:(state.S+1)%4});}
...
main{height:100dvh;max-height:100dvh;width:100%;padding:7px 10px 8px;display:flex;flex-direction:column;gap:7px;overflow:hidden}.head{height:34px;flex:0 0 34px;display:flex;align-items:flex-end;justify-content:space-between}h1{margin:0;font-size:25px;line-height:1}.sub{font-size:11px;color:#8e8e93}.status{height:62px;flex:0 0 62px;background:#111;border-radius:18px;padding:10px 16px;display:grid;grid-template-columns:1fr 1fr;align-items:center}.mode{font-size:20px;font-weight:800}.temp{font-size:31px;font-weight:800;text-align:right}.controls{flex:1;min-height:0;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:repeat(4,minmax(0,1fr));gap:7px}button{min-height:0;border:0;border-radius:19px;background:#171717;color:#0a84ff;font-size:20px;font-weight:780;touch-action:manipulation}button:active{background:#2a2a2a;transform:scale(.985)}button.stop{color:#ff453a}button:disabled{opacity:.35}
@media(max-height:720px){main{padding:5px 8px 6px;gap:5px}.head{height:28px;flex-basis:28px}h1{font-size:22px}.status{height:52px;flex-basis:52px;border-radius:15px}.mode{font-size:17px}.temp{font-size:27px}.controls{gap:5px}button{font-size:17px;border-radius:16px}}
