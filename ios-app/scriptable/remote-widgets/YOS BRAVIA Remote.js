...
const state={host,count:remoteMap.size,support:Object.fromEntries(Object.keys(ACTIONS).map(a=>[a,support(a)])),commands:[...remoteMap.keys()].filter(n=>!/flashminus|flashplus/i.test(String(n))).map(n=>({name:n,label:label(n)})).sort((a,b)=>a.label.localeCompare(b.label,'ja')),layout:loadLayout(),defaultLayout:[...DEFAULT_LAYOUT],items:UI_ITEMS};
...
</style></head><body><main><div class="head"><h1>BRAVIA</h1><div class="meta">接続済み · ${state.count}</div></div><div id="top" class="top"></div><div id="pad" class="pad"><div id="seekStatus" class="seekStatus"></div><div id="nub" class="nub"></div><div class="hint">タップ＝決定　短スワイプ＝移動　横へ長押し＝動画操作</div></div><div id="grid" class="grid"></div></main>
...
const pad=document.getElementById('pad'),nub=document.getElementById('nub'),seekStatus=document.getElementById('seekStatus');
let start=null,last=null,seekDir=null,seekStage=0,seekTimers=[],navRepeatStart=null,navRepeatTimer=null,navRepeatDir=null,navRepeated=false;
function clearSeekTimers(){seekTimers.forEach(t=>clearTimeout(t));seekTimers=[]}
function clearNavRepeat(){if(navRepeatStart)clearTimeout(navRepeatStart);if(navRepeatTimer)clearInterval(navRepeatTimer);navRepeatStart=null;navRepeatTimer=null;navRepeatDir=null}
function resetNub(){nub.style.transform='translate(0px,0px)'}
function resetGesture(){clearSeekTimers();clearNavRepeat();start=null;last=null;seekDir=null;seekStage=0;navRepeated=false;resetNub();seekStatus.textContent=''}
function seekLabel(dir,stage){return (dir==='forward'?'▶▶ 早送り':'◀◀ 巻戻し')+' ×'+stage}
function armSeek(dir){if(!start||!STATE.support[dir])return;if(seekDir===dir&&seekTimers.length)return;clearNavRepeat();clearSeekTimers();seekDir=dir;const elapsed=Date.now()-start.t;[{ms:450,stage:2},{ms:1450,stage:3},{ms:2650,stage:4}].forEach(step=>{const delay=Math.max(0,step.ms-elapsed);const timer=setTimeout(()=>{if(!start||seekDir!==dir)return;seekStage=step.stage;seekStatus.textContent=seekLabel(dir,step.stage);bridge('seek-step',{name:dir,stage:step.stage})},delay);seekTimers.push(timer)})}
function armNavRepeat(dir){if(!start||!STATE.support[dir]||seekDir)return;if(navRepeatDir===dir)return;clearNavRepeat();navRepeatDir=dir;seekStatus.textContent='カーソル移動';navRepeatStart=setTimeout(()=>{if(!start||seekDir||navRepeatDir!==dir)return;navRepeated=true;bridge('action',{name:dir});navRepeatTimer=setInterval(()=>{if(!start||seekDir||navRepeatDir!==dir){clearNavRepeat();return}bridge('action',{name:dir})},170)},360)}
function updateGesture(x,y){if(!start)return;last={x,y};const dx=Math.max(-95,Math.min(95,x-start.x)),dy=Math.max(-58,Math.min(58,y-start.y));nub.style.transform='translate('+dx+'px,'+dy+'px)';const rawDx=x-start.x,rawDy=y-start.y,ax=Math.abs(rawDx),ay=Math.abs(rawDy);if(seekDir)return;if(ax>60&&ax>ay*1.15){armSeek(rawDx>0?'forward':'rewind');return}if(Math.max(ax,ay)>28){const dir=ax>ay?(rawDx>0?'right':'left'):(rawDy>0?'down':'up');armNavRepeat(dir)}else clearNavRepeat()}
function beginGesture(x,y){start={x,y,t:Date.now()};last={x,y};seekDir=null;seekStage=0;navRepeated=false;clearSeekTimers();clearNavRepeat();seekStatus.textContent=''}
function finishGesture(x,y){if(!start)return;const p=last||{x,y},dx=p.x-start.x,dy=p.y-start.y,ax=Math.abs(dx),ay=Math.abs(dy);clearSeekTimers();clearNavRepeat();if(seekStage>0){if(STATE.support.play)bridge('action',{name:'play'});resetGesture();return}if(navRepeated){resetGesture();return}if(ax<22&&ay<22)bridge('action',{name:'confirm'});else if(ax>ay)bridge('action',{name:dx>0?'right':'left'});else bridge('action',{name:dy>0?'down':'up'});resetGesture()}
...
pad.addEventListener('touchcancel',e=>{if(!start)return;e.preventDefault();if(seekStage>0&&STATE.support.play)bridge('action',{name:'play'});resetGesture()},{passive:false});
pad.addEventListener('contextmenu',e=>e.preventDefault());
document.addEventListener('visibilitychange',()=>{if(document.hidden&&start){if(seekStage>0&&STATE.support.play)bridge('action',{name:'play'});resetGesture()}});
</script></body></html>`;
