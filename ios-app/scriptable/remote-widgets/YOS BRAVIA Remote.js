...
const state={
  ...
  // Remove the dedicated ±10/15 commands from the visible "Other" list too.
...
main{height:100dvh;max-height:100dvh;padding:5px 8px 6px;display:flex;flex-direction:column;gap:5px;overflow:hidden}.head{height:27px;flex:0 0 27px;display:flex;align-items:flex-end;justify-content:space-between}h1{margin:0;font-size:22px;line-height:1}.meta{font-size:9px;color:#8e8e93;white-space:nowrap}.top{height:52px;flex:0 0 52px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.pad{height:clamp(104px,17dvh,132px);flex:0 0 clamp(104px,17dvh,132px);border-radius:20px;border:1px solid #343434;background:#111;position:relative;touch-action:none;overflow:hidden}.nub{width:48px;height:48px;border-radius:50%;border:1px solid #74b9ff;background:#38577d;position:absolute;left:50%;top:50%;margin:-24px;pointer-events:none;transition:transform .05s linear}.hint{position:absolute;left:0;right:0;bottom:7px;text-align:center;color:#5d5d62;font-size:9px;pointer-events:none}.seekStatus{position:absolute;left:0;right:0;top:7px;text-align:center;color:#ffcc66;font-size:11px;font-weight:800;pointer-events:none;min-height:14px}.grid{flex:1;min-height:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));grid-template-rows:repeat(5,minmax(0,1fr));gap:5px}button{min-width:0;min-height:0;border:0;border-radius:13px;background:#171717;color:#0a84ff;font-size:14px;font-weight:750;padding:2px;touch-action:manipulation}button:active{background:#2a2a2a;transform:scale(.985)}button.power{color:#ff453a}button:disabled{opacity:.25}.overlay{position:fixed;inset:0;background:rgba(0,0,0,.76);display:none;align-items:flex-end;z-index:20}.sheet{width:100%;max-height:84dvh;background:#151515;border-radius:22px 22px 0 0;padding:12px 12px 22px;overflow:auto}.sheetHead{display:flex;align-items:center;justify-content:space-between;position:sticky;top:-12px;background:#151515;padding:8px 0;z-index:2}.sheetHead h2{margin:0;font-size:19px}.close{height:38px;padding:0 14px;color:#fff;background:#2b2b2b}.commandList{display:grid;grid-template-columns:1fr 1fr;gap:7px}.commandList button{height:44px;font-size:13px}.settingsText{color:#8e8e93;font-size:11px;line-height:1.45;margin:2px 0 10px}.settingsTitle{font-size:12px;font-weight:800;margin:10px 0 6px;color:#d1d1d6}.editTop{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.editGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}.editTop button,.editGrid button{height:42px;font-size:11px}.editTop button.selected,.editGrid button.selected{outline:2px solid #0a84ff;background:#26374a}.settingsActions{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:10px}.settingsActions button{height:44px;font-size:13px}.primary{background:#0a84ff!important;color:#fff!important}.danger{color:#ff9f0a!important}
@media(max-height:720px){main{padding:4px 7px 5px;gap:4px}.head{height:23px;flex-basis:23px}h1{font-size:19px}.meta{font-size:8px}.top{height:44px;flex-basis:44px;gap:4px}.pad{height:84px;flex-basis:84px;border-radius:16px}.nub{width:40px;height:40px;margin:-20px}.grid{gap:4px}button{font-size:12px;border-radius:11px}.hint{font-size:8px;bottom:4px}.seekStatus{top:4px;font-size:9.5px}}
...
function updateGesture(x,y){if(!start)return;last={x,y};const dx=Math.max(-66,Math.min(66,x-start.x)),dy=Math.max(-42,Math.min(42,y-start.y));nub.style.transform='translate('+dx+'px,'+dy+'px)';const rawDx=x-start.x,rawDy=y-start.y,ax=Math.abs(rawDx),ay=Math.abs(rawDy);if(ax>60&&ax>ay*1.15)armSeek(rawDx>0?'forward':'rewind');else if(ay>ax||ax<46){if(seekStage===0)cancelSeekArm()}}
