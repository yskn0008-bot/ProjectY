...
pad.addEventListener('pointerup',endGesture);pad.addEventListener('pointercancel',()=>{if(seekStage>0&&STATE.support.play)bridge('action',{name:'play'});resetGesture()});document.addEventListener('visibilitychange',()=>{if(document.hidden&&start){if(seekStage>0&&STATE.support.play)bridge('action',{name:'play'});resetGesture()}});
</script></body></html>`;
