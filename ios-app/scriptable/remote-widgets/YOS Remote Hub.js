...
const TV_ACTIONS = [
  ...
  { action: "quick", icon: "⚙", label: "クイック", color: "blue" },
...
function runURL(name) {
  return "scriptable:///run?scriptName=" + encodeURIComponent(name)
}
...
function makeButtons(device, items) {
  return items.map(item =>
    '<button class="key" onclick="sendOther(event,\'' +
    device + "','" + item[0] + "','" + item[2] + "')">" +
    '<span class="icon">' + item[1] + "</span>" +
    '<span class="label">' + item[2] + "</span></button>"
  ).join("")
}
...
.icon{font-size:25px;color:#438eff}
.label{font-size:12px;font-weight:700;white-space:nowrap}
.controls{margin-top:10px}
.dpad{width:190px;height:154px;margin:0 auto;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);gap:6px}
.dpad .key{min-height:0;align-items:center}
.up{grid-column:2}
.left{grid-column:1;grid-row:2}
.ok{grid-column:2;grid-row:2;border-radius:50%;background:#24384f}
.right{grid-column:3;grid-row:2}
.down{grid-column:2;grid-row:3}
.media{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}
.media .key{min-height:52px;align-items:center}
...
<section class="card">
  <div class="head">
    ...
    <div class="tools"><button class="tool" onclick="sendMenu(event)">MENU</button></div>
  </div>
  <div id="tvGrid" class="grid grid-3"></div>
  <div id="controls" class="controls"></div>
</section>
<section class="card">
  <div class="head"><div class="title" onclick="openRemote('light')">照明</div><div class="brand">Panasonic</div></div>
  <div class="grid grid-3">${makeButtons("light", LIGHT_BUTTONS)}</div>
</section>
<section class="card">
  <div class="head"><div class="title" onclick="openRemote('ac')">エアコン</div><div class="brand">SHARP</div></div>
...
const LEGACY=${JSON.stringify(LEGACY_DEFAULT)}
const KEY="yos.remote.tv.buttons.v52"
...
function valid(action){return ACTIONS.some(item=>item.action===action)}
function same(a,b){return a.length===b.length&&a.every((value,index)=>value===b[index])}

function loadLayout(){
  let saved=[]
  try{saved=JSON.parse(localStorage.getItem(KEY))}catch(_){}
  let layout=Array.isArray(saved)?saved.slice(0,12).filter(valid):[]
  if(same(layout,LEGACY))layout=DEFAULT.slice()
  else if(layout.length===12&&!layout.includes("quick")){
    const power=layout.indexOf("power")
    if(power!==-1)layout[power]="quick"
  }
  for(const action of DEFAULT){
    if(layout.length>=12)break
    if(!layout.includes(action))layout.push(action)
  }
  for(const item of ACTIONS){
    if(layout.length>=12)break
    if(!layout.includes(item.action))layout.push(item.action)
  }
  return layout.slice(0,12)
}

const layout=loadLayout()
function meta(action){return ACTIONS.find(item=>item.action===action)}

function nativeAction(device,action,label){
  if(device==="tv"){
  ...
  window.location.href="/__action?device="+encodeURIComponent(device)+
    ...
    "&label="+encodeURIComponent(label)+"&n="+nonce
}

function makeKey(action,extra=""){
  const item=meta(action)
  const button=document.createElement("button")
  button.className="key "+extra
  button.innerHTML='<span class="icon">'+item.icon+
    '</span><span class="label">'+item.label+"</span>"
  button.onclick=event=>{
    event.preventDefault()
    event.stopPropagation()
    nativeAction("tv",action,item.label)
  }
  return button
}
...
function renderTV(){
  ...
  root.innerHTML=""
  layout.forEach(action=>root.appendChild(makeKey(action)))
}
...
function renderControls(){
  ...
  if(mode==="media"){
    ...
    ;["prev","play","next","rewind","pause","fastForward","stop"].forEach(
      action=>media.appendChild(makeKey(action))
    )
  ...
  ].forEach(item=>pad.appendChild(makeKey(item[0],item[1])))
  root.appendChild(pad)
}

function setMode(value){
  if(value==="navigation"||value==="media"){
    mode=value
    renderControls()
  }
}
...
function sendMenu(event){
  event.preventDefault()
  event.stopPropagation()
  nativeAction("tv","menu","MENU")
}

function sendOther(event,device,action,label){
  event.preventDefault()
  event.stopPropagation()
  nativeAction(device,action,label)
}
...
function sonyHost() {
  const host = secure(STORAGE.host)
    .trim()
    .replace(/^https?:\/\//i, "")
    .replace(/:\d+$/, "")
  if (!host) throw new Error("BRAVIA IP設定なし")
  return host
}

function sonyPSK() {
  const value = secure(STORAGE.psk)
  if (!value) throw new Error("BRAVIA PSK設定なし")
  return value
}
...
async function sonyJSON(service, method) {
  const request = new Request("http://" + sonyHost() + "/sony/" + service)
  request.method = "POST"
  request.headers = {
    "Content-Type": "application/json",
    "X-Auth-PSK": sonyPSK()
  }
  request.body = JSON.stringify({ method, params: [], id: 1, version: "1.0" })
...
async function sonyAction(action) {
  ...
  if (action === "quick" && Keychain.contains(STORAGE.quick)) {
    ...
    if (commandIndex.has(saved)) code = commandIndex.get(saved)
  }
  for (const alias of ALIASES[action] || []) {
    if (!code && commandIndex.has(alias.toLowerCase())) {
      code = commandIndex.get(alias.toLowerCase())
    }
  }
  if (!code) throw new Error("BRAVIA未対応：" + action)

  const request = new Request("http://" + sonyHost() + "/sony/ircc")
  request.method = "POST"
  request.headers = {
...
function hasNavigationStatus(data) {
  ...
  return Array.isArray(list) && list.some(item =>
    item && ["cursorDisplay", "textInput", "webBrowse"].some(key => {
...
async function readScript(scriptName) {
  for (const manager of [FileManager.iCloud(), FileManager.local()]) {
    const directory = manager.documentsDirectory()
    let files = []
    try { files = manager.listContents(directory) } catch (_) {}
    for (const file of files) {
      if (
        normalizeName(file) !== normalizeName(scriptName) &&
        normalizeName(file) !== normalizeName(scriptName + ".js")
      ) continue
      const path = manager.joinPath(directory, file)
      try {
        if (manager.isFileStoredIniCloud(path)) {
          await manager.downloadFileFromiCloud(path)
        }
      } catch (_) {}
      return manager.readString(path)
    }
  }
...
async function runExisting(scriptName, action) {
  ...
  await new AsyncFunction("args", "Script", "config", source)(
    fakeArgs, fakeScript, fakeConfig
  )
}

function parseQuery(url) {
  const output = {}
  const position = String(url).indexOf("?")
  if (position < 0) return output
  String(url).slice(position + 1).split("&").forEach(pair => {
...
async function processQueue() {
  ...
  while (queue.length) {
    ...
    try {
      await web.evaluateJavaScript("nativeResult(" + JSON.stringify(result) + ")")
    } catch (_) {}
  }
  running = false
}
...
try {
  await web.evaluateJavaScript(
    "setMode(" + JSON.stringify(await inferMode()) + ")"
  )
} catch (_) {}
