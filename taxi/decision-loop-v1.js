(function expose(root, factory) {
  ...
  function buildDecision(input = {}) {
    ...
    if (status.toLowerCase() === "occupied" || input.occupied === true) {
      ...
      return { action: "安全に運行を継続する", recommendedArea: null, mode: "safe", cutoffMinutes: null,
    ...
    if (demand) {
      ...
      return { action: `${area}で需要を確認しながら営業する`, recommendedArea: area,
        ...
        reasons: ["confirmed/highの需要が現在条件に一致"], confidence: "high", facts, inferences, unverified };
    }

    unverified.push(input.demandSelection || input.demand
      ? "需要情報はconfirmed/high・現在時刻・確定エリアの一致を確認できない" : "現在利用できる需要情報");
    inferences.push("根拠不足のため特定エリアへの移動は断定しない");
    return { action: "安全な場所で状況を確認する", recommendedArea: null, mode: "wait", cutoffMinutes: 15,
  ...
  function buildRideContext(input = {}) {
    ...
    if (decision && typeof decision === "object") result.decisionContext = {
      action: text(decision.action), recommendedArea: text(decision.recommendedArea),
      mode: MODES.has(decision.mode) ? decision.mode : null, cutoffMinutes: number(decision.cutoffMinutes),
      confidence: text(decision.confidence), reasons: Array.isArray(decision.reasons) ? decision.reasons.map(text).filter(Boolean) : [],
  ...
  function analyzeShift(state) {
    const source = Array.isArray(state) ? {} : state && typeof state === "object" ? state : {};
    const events = Array.isArray(state) ? state : Array.isArray(source.events) ? source.events : [];
    const rides = events.filter((event) => {
      const kind = text(event && (event.type || event.kind || event.event));
      return kind === "降車" || ["dropoff", "ride_complete", "fare"].includes(kind?.toLowerCase());
    });
    let revenue = 0;
    let vacantMinutes = 0;
    ...
    const byHour = {};
    const byArea = {};
    const byDemandContext = {};
    const add = (groups, key, amount) => {
    ...
    for (const event of rides) {
      ...
      const amount = fare + tip;
      revenue += amount;
      const wait = number(event.waitMs) !== null ? number(event.waitMs) / 60000 : number(event.idleMinutes ?? event.vacantMinutes);
      const duration = number(event.durationMs) !== null ? number(event.durationMs) / 60000 : number(event.rideMinutes ?? event.occupiedMinutes);
      if (wait !== null) vacantMinutes += Math.max(0, wait);
      if (duration !== null) occupiedMinutes += Math.max(0, duration);
      const at = date(event.at || event.time || event.timestamp || event.dropoffAt || event.endedAt);
      if (at) add(byHour, String(at.getHours()).padStart(2, "0"), amount);
      const area = event.areaConfirmed === false ? null : text(event.area);
      if (area) add(byArea, area, amount);
      const demand = event.demandContext;
      const demandKey = demand && typeof demand === "object" ? text(demand.id) || text(demand.title) : null;
      if (demandKey) add(byDemandContext, demandKey, amount);
    }
    const started = date(source.shiftStart || source.startedAt || source.startTime);
    const ended = date(source.shiftEnd || source.endedAt || source.endTime);
    ...
    return { dataAvailable: rides.length > 0, revenue, rideCount: rides.length,
      ...
      utilizationRate: tracked ? occupiedMinutes / tracked : null, vacantMinutes,
