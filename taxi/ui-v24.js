...
(()=>{
  ...
  const THEMES={
    ...
    taxi511:{label:'511 Taxi',description:'白・オレンジ・濃紺',swatches:['#f7f2e9','#d9681b','#173f91']},
  ...
  let prefs={...read(KEY,{}),pageOrder:safeOrder(read(KEY,{}).pageOrder),appearance:read(KEY,{}).appearance||'auto',theme:THEMES[read(KEY,{}).theme]?read(KEY,{}).theme:'gold'};
  ...
  function buildBottomNav(){
    ...
    if(!nav){nav=document.createElement('nav');nav.id='taxiGlobalNavV24';nav.className='taxi-global-nav-v24';document.body.appendChild(nav)}
  ...
  function installModeButton(){
    ...
    const button=document.createElement('button');button.id='taxiModeQuickV24';button.type='button';button.className='taxi-mode-quick-v24';button.setAttribute('aria-label','昼夜モードを切り替える');
    const last=top.lastElementChild;last?top.insertBefore(button,last):top.appendChild(button);
    button.onclick=()=>{prefs.appearance=prefs.appearance==='auto'?'light':prefs.appearance==='light'?'dark':'auto';persist();applyTheme()};
    paintControls();
  }

  function paintControls(){
    const quick=document.getElementById('taxiModeQuickV24');
    if(quick){quick.textContent=prefs.appearance==='light'?'☀️':prefs.appearance==='dark'?'🌙':'◐';quick.title=prefs.appearance==='light'?'昼モード':prefs.appearance==='dark'?'夜モード':'自動モード'}
  ...
  function displayPanel(){
    const panel=document.createElement('section');panel.id='taxiDisplayPanelV24';panel.className='panel';panel.dataset.settingsGroup='display';
    panel.innerHTML=`<h2>画面表示 <small>ページ順・昼夜モード・配色を変更します。</small></h2>
      ...
      <div class="taxi-setting-block-v24"><div class="taxi-setting-label-v24">下部ページの順番</div><div id="taxiPageOrderV24" class="taxi-order-list-v24"></div></div>`;
    panel.querySelectorAll('#taxiAppearanceV24 button').forEach(button=>button.onclick=()=>{prefs.appearance=button.dataset.value;persist();applyTheme()});
    panel.querySelectorAll('#taxiThemesV24 button').forEach(button=>button.onclick=()=>{prefs.theme=button.dataset.theme;persist();applyTheme()});
    return panel;
  }

  function renderOrderControls(){
    const box=document.getElementById('taxiPageOrderV24');if(!box)return;
    ...
    box.querySelectorAll('button').forEach(button=>button.onclick=()=>{
      const row=button.closest('[data-key]'),index=prefs.pageOrder.indexOf(row.dataset.key),next=button.dataset.dir==='up'?index-1:index+1;
      ...
      persist();renderOrderControls();buildBottomNav();
    });
  }
  ...
  function installSettingsDisplay(){
    ...
    const wait=setInterval(()=>{
      const tabs=document.getElementById('settingsTabsV20');const app=document.querySelector('main.app');
      if(!tabs||!app)return;
      clearInterval(wait);
      ...
      const button=document.createElement('button');button.type='button';button.dataset.tab='display';button.textContent='表示';tabs.appendChild(button);
      button.onclick=()=>{
      ...
      renderOrderControls();paintControls();
    },60);
  }
  ...
  function installCalendarQuery(){
    ...
    const wait=setInterval(()=>{if(activateCalendarPage(requested,false)){clearInterval(wait);updateNavActive(requested)}},60);
  ...
  setInterval(()=>{if(prefs.appearance==='auto')applyTheme();updateNavActive()},30000);
})();
