'use strict';
const CACHE='yos-taxi-projecty-v15-1';
const STATIC=['./calendar.html','./settings.html','./manifest.webmanifest','./v9.css','./v9.js','./calendar-v2.js','./swipe-nav.js','./v15.js'];
...
async function inject(response,type){
  ...
  if(type==='calendar'){
    if(!html.includes('calendar-v2.js'))html=html.replace('</body>','<script src="./calendar-v2.js?v=15.1"></script></body>');
    if(!html.includes('v15.js'))html=html.replace('</body>','<script src="./v15.js?v=15.1"></script></body>');
    if(!html.includes('swipe-nav.js'))html=html.replace('</body>','<script src="./swipe-nav.js"></script></body>');
  }
