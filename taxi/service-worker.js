'use strict';
const CACHE='yos-taxi-projecty-v12';
const STATIC=['./calendar.html','./settings.html','./manifest.webmanifest','./v9.css','./v9.js','./calendar-v2.js','./swipe-nav.js'];
...
async function inject(response,type){
  ...
  if(type==='calendar'){
    if(!html.includes('calendar-v2.js'))html=html.replace('</body>','<script src="./calendar-v2.js"></script></body>');
    if(!html.includes('swipe-nav.js'))html=html.replace('</body>','<script src="./swipe-nav.js"></script></body>');
  }
...
self.addEventListener('install',event=>event.waitUntil((async()=>{
  ...
  for(const path of ['./','./index.html','./calendar.html']){
    const response=await fetch(path),type=path.includes('calendar')?'calendar':'index';
...
self.addEventListener('fetch',event=>{
  ...
  if(type){event.respondWith(fetch(event.request).then(async response=>{const transformed=await inject(response,type),copy=transformed.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy));return transformed}).catch(()=>caches.match(event.request).then(hit=>hit||caches.match(type==='calendar'?'./calendar.html':'./index.html'))));return}
